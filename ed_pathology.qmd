...
setwd("/Volumes/kschaumberg")
load("/Volumes/kschaumberg/BAM/BAM-data-cleaning/data/BAM_redcap_long_merged.2023-08-11.RData")
 load("/Volumes/kschaumberg/BAM/BAM-data-cleaning/data/BAM_redcap_wide_merged.2023-08-11.RData")
```

```{r message=FALSE, echo=FALSE, warning=FALSE}
#Split the dataframe by group and get FP only
...
```{r message=FALSE, echo=FALSE, warning=FALSE}
#New DF with just the subscales I want
...
inter_g_long$means <- as.numeric(as_vector(inter_g_long$means))
...
epsi_g <- ggplot(data = inter_g_long, aes(x = timepoint, y = means, group =subscale))+ geom_point()+ geom_line(aes( color = subscale))+ labs(x = "Timepoint", y = "Mean EPSI Subscale Score", color = "Subscale")+ theme_igray()+ scale_x_discrete(labels = c("1" = "Baseline", "2" = "8 Week")) + scale_color_discrete(labels = c("binge" = "Binge Eating", "body_dissat" = "Body Dissatisfaction", "exercise" = "Excessive Exercise", "muscle_build" = "Muscle Building", "purging" = "Purging", "restraint" = "Cognitive Restraint", "restrict" = "Restricting" ))+  
  geom_text(
...
inter_g$cohens_d <- c(cohen.d(fp_epsi$body_dissatisfaction_baseline,fp_epsi$body_dissatisfaction_8wk)[[3]],
                  cohen.d(fp_epsi$binge_baseline,fp_epsi$binge_8wk)[[3]],
                  cohen.d(fp_epsi$restraint_baseline,fp_epsi$restraint_8wk)[[3]],
                  cohen.d(fp_epsi$purging_baseline,fp_epsi$purging_8wk)[[3]],
                  cohen.d(fp_epsi$restrict_baseline,fp_epsi$restrict_8wk)[[3]],
                  cohen.d(fp_epsi$exercise_baseline,fp_epsi$exercise_8wk)[[3]],
                  cohen.d(fp_epsi$muscle_build_baseline,fp_epsi$muscle_build_8wk)[[3]])
test <-cohen.d(fp_epsi$body_dissatisfaction_baseline,fp_epsi$body_dissatisfaction_8wk)
```